    ...
    for file_info in config["files"]
        ...
        for (file_grp, internal_grp) in groups
            for (grp_key, grp_val) in internal_grp
                try
                    all_data[grp_val] = ds.group[file_grp][grp_key][:]
                catch e
...
geos_data = loadGeos("/Users/arpitasen/Documents/GitHub/EMIT-Caltech/src/EMIT.yaml")

rad = geos_data.data["radiance"]
latitude = geos_data.data["lat"]
longitude = geos_data.data["lon"]
